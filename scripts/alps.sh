...
RUN_ALL_STEPS="FALSE"
RUN_AMD_ONLY="FALSE"
COMPILE_SERVER_CODE="FALSE"
ADAPTER_SHOW_SQL="false"
DROP_FIRST="false"
RUN_MVN="mvn clean install -Dadapter_show_sql=$ADAPTER_SHOW_SQL"
...
usage()
{
	...
	echo ""
	echo "[-c Compile server] compileing lucas-api and lucas-services"
	echo ""
...
while getopts "abcfilmnqsx?" 2> /dev/null ARG
do
	case $ARG in
...
RUN_MVN="$RUN_MVN -DdropFirst=$DROP_FIRST"
...
then
	run_skip_test
elif [ "$BUILD_UNIT=" = "TRUE" ]
then
	run_build_unit
elif [ "$RUN_WITH_LIQUIBASE" = "TRUE" ]
then
	run_with_liquibase
elif [ "$RUN_NO_DB" = "TRUE" ]
then
